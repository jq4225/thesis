...
zero_percent <- searches_norm3_v4 %>%
  group_by(dma) %>%
  select(dma, `alcohol`:`worry`, depression, cannabis:yoga) %>% 
  summarize(across(.cols = c(`alcohol`:`yoga`), ~ sum(.==0)),
            weeks = n()) %>%
  mutate(across(.cols = c(alcohol:yoga), ~ . * 100 / weeks)) %>%
  group_by(dma) %>%
  mutate(mean = mean(alcohol:yoga))

zero_percent_summary <- zero_percent %>%
  ungroup() %>%
  summarize(across(.cols = alcohol:yoga, ~ mean(., na.rm = TRUE))) %>%
  pivot_longer(cols = alcohol:yoga, names_to = "term", values_to = "percent_zero")
