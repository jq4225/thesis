...
for(i in 1:54) {
  ...
  if(i == 1) {
    ...
    cigarette <- cbind(cigarette, temp)
  }
  Sys.sleep(3)
...
cigarette_clean <- cigarette[, -which(duplicated(names(cigarette)))] %>%
  select(-`US-NY-501`) %>%
  rename(`US-NY-501` = `US-NY-501-FINAL`) %>% 
  pivot_longer(cols = `US-AK-743`:`US-NY-501`,
               names_to = "dma", values_to = "cigarette")

saveRDS(cigarette_clean, file = "cigarette_scaled.rds")
```
