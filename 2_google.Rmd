...
for(i in 1:54) {
  ...
  if(i < 53) {
    ...
    if(i != 32) {
       ...
       temp <- temp %>% pivot_wider(id_cols = date, names_from = geo,
                                    values_from = hits) %>%
...
stress_clean <- stress[, -which(duplicated(names(stress)))] %>%
  select(-`US-NY-501`) %>%
  rename(`US-NY-501` = `US-NY-501-FINAL`)

saveRDS(stress_clean, file = "stress_scaled.rds")
```
```{r}
# Pivot longer really quickly

depression_long <- depression_scaled %>%
  pivot_longer(cols = `US-AK-743`:`US-NY-501`,
               names_to = "dma", values_to = "depression")

saveRDS(depression_long, file = "depression_scaled.rds")
...
for(j in 1:length(anchor_list)) {
  ...
  keyword_clean <- keyword[, -which(duplicated(names(stress)))] %>%
    select(-`US-NY-501`) %>%
    rename(`US-NY-501` = `US-NY-501-FINAL`) %>%
    pivot_longer(cols = `US-AK-743`:`US-NY-501`,
                 names_to = "dma", values_to = anchor[2,j])
  
  if(j == 1) {
    anchor_index <- keyword
  }
  
  else {
    anchor_index <- left_join(anchor_index, keyword, by = c("dma", "date"))
  }
