...
for(i in 1:54) {
  ...
  if(i == 1) {
    ...
    loneliness <- cbind(loneliness, temp)
  }
  Sys.sleep(3)
...
loneliness_clean <- loneliness[, -which(duplicated(names(loneliness)))] %>%
  select(-`US-NY-501`) %>%
  rename(`US-NY-501` = `US-NY-501-FINAL`)

saveRDS(loneliness_clean, file = "loneliness_scaled.rds")
```
...
word <- gtrends(keyword = 'loneliness', geo = c('US-NY-501', 'US-AL-606'),
                            time = '2016-01-01 2020-12-31', 
                         onlyInterest = TRUE)$interest_over_time
