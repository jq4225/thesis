...
lockdown_clean <- lockdown[, -which(duplicated(names(lockdown)))] %>%
  select(-`US-NY-501`) %>%
  rename(`US-NY-501` = `US-NY-501-FINAL`) %>% 
  pivot_longer(cols = `US-AK-743`:`US-NY-501`,
               names_to = "dma", values_to = "lockdown")

saveRDS(lockdown_clean, file = "lockdown_scaled.rds")
```
