...
tired_clean <- tired[, -which(duplicated(names(tired)))] %>%
  select(-`US-NY`) %>%
  rename(`US-NY` = `US-NY-FINAL`) %>% 
  pivot_longer(cols = `US-AL`:`US-NY`,
               names_to = "state", values_to = "tired")

saveRDS(tired_clean, file = "tired_state.rds")
```


```{r}
# Looped version for multiple terms

search_term <- c("nervous", "stress", "worry", "panic")

search_meaning <- search_term

# search_term <- c('/m/0463cq4', 'lonely', 'sad', '/m/06z5s')
# 
# search_meaning <- c('crying', 'lonely', 'sad', 'suicide')
...
for(j in 1:length(search_term)) {
  for(i in 1:14) {
    ...
    if(i < 13) {
      # Exception case for when we need to search twice for NY
      if(i != 8) {
         temp <- gtrends(keyword = search_term[j], geo = c('US-NY', state_codes[(i-1)*4 + 1],
                                                   state_codes[(i-1)*4 + 2],
                                                   state_codes[(i-1)*4 + 3],
                                                   state_codes[(i-1)*4 + 4]),
                              time = '2020-04-19 2021-09-27', 
      ...
      temp <- gtrends(keyword = search_term[j], geo = c('US-NY'),
                              time = '2020-04-19 2021-09-27', 
                           onlyInterest = TRUE)$interest_over_time %>%
        mutate(hits = ifelse(hits == "<1", "0.5", hits)) %>%
        mutate(hits = as.numeric(hits)) %>%
  ...
  total_clean <- total[, -which(duplicated(names(total)))] %>%
    select(-`US-NY`) %>%
    rename(`US-NY` = `US-NY-FINAL`) %>% 
    pivot_longer(cols = `US-AL`:`US-NY`,
                 names_to = "state", values_to = paste(search_meaning[j]))
  
  if(j == 1) {
      total_total <- total_clean
  }
  else {
      total_total <- left_join(total_total, total_clean, by = c("date", "state"))
  }

  if(j == length(search_term)) {
    saveRDS(total_total, file = "anxiety_terms_state.rds")
...
search_term <- "/m/012jc" # alcoholism
...
# alcoholism 
alcohol_v2 <- alcohol_final %>%
  left_join(alcohol_clean %>% rename("alcoholism" = "alcohol")) %>%
  mutate(alcoholism_norm = alcoholism/average) %>% 
  mutate(alcoholism_norm = alcoholism_norm * 100 / max(alcoholism_norm))
```

```{r}
search_term <- "/m/02vxs3q" # smoking

search_term <- "/m/09k76" # cigarettes

search_term <- "/m/02vlckk" # e-cigarettes
...
```{r}
# Adding in the new smoking searches

smoking_v2 <- smoking_final %>%
  left_join(smoking_clean %>% rename("cigarettes" = "smoking")) %>% 
  mutate(cigarettes_norm = cigarettes / average) %>% 
  mutate(cigarettes_norm = cigarettes_norm * 100 / max(cigarettes_norm))

smoking_v3 <- smoking_v2 %>%
  left_join(smoking_clean %>% rename("e_cig" = "smoking")) %>% 
  mutate(e_cig_norm = e_cig / average) %>% 
  mutate(e_cig_norm = e_cig_norm * 100 / max(e_cig_norm))
```

```{r}
# Adding in addiction

search_term <- "/m/0l08" # addiction

search_term <- "/m/0278r2" # drug rehab
...
for(i in 1:54) {
  ...
  if(i == 1) {
    ...
    addiction <- cbind(addiction, temp)
  }
  Sys.sleep(3)
...
addiction_clean <- addiction[, -which(duplicated(names(addiction)))] %>%
  select(-`US-NY-501`) %>%
  rename(`US-NY-501` = `US-NY-501-FINAL`) %>% 
  pivot_longer(cols = `US-AK-743`:`US-NY-501`,
               names_to = "dma", values_to = "addiction")

saveRDS(addiction_clean, file = "addiction_scaled.rds")
```
...
addiction_clean <- addiction_clean %>%
  rename('rehab' = 'addiction')

searches_norm3_v20 <- searches_norm3_v19 %>%
  left_join(addiction_clean, by = c("dma", "date")) %>% 
  mutate(rehab = rehab / average_3) %>% 
  mutate(rehab = rehab * 100 / max(rehab))

searches_norm3_v20_5 <- searches_norm3_v20 %>%
  filter(dma %in% dma_5)

searches_v24 <- searches_v23 %>%
  left_join(addiction_clean, by = c("dma", "date"))

searches_v24_5 <- searches_v24 %>%
  filter(dma %in% dma_5)
