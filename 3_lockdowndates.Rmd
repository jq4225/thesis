...
school <- read_excel('data/policy/covid-school-data.xlsx') %>% 
  ...
  select(district_nces,
         physical_state, open_date, teaching_method)

crosswalkteahi <- read_excel('data/policy/sdlist-20.xls', skip = 2) %>% 
  clean_names() %>% 
...
reopening <- school_2 %>% 
  ...
  pivot_wider(names_from = teaching_method,
              values_from = n) %>% 
  clean_names() %>% 
  mutate(across(hybrid:other, ~ replace_na(., 0))) %>% 
  mutate(districts = hybrid + online_only + on_premises + other)

dates <- school_2 %>% 
  select(google_code, open_date) %>% 
  mutate(open_date = mdy(open_date)) %>% 
  group_by(google_code) %>% 
  summarize(min = min(open_date),
            max = max(open_date),
            average = mean(open_date),
            sd = sd(open_date))
