...
hps_all_v6 %>% 
  ...
  summarize(across(c(hps_depression, depression_norm, crying_norm:sad_norm),
                   ~ weighted.mean(., pop))) %>% 
  rowwise() %>% 
  mutate(google_average = mean(depression_norm:sad_norm)) %>% 
  ungroup() %>% 
  ggplot(aes(x = start_date)) + 
    geom_line(aes(y = hps_depression/1.2, color = "Household Pulse")) + 
    geom_line(aes(y = google_average/3, color = "Avg. related SVIs (re-scaled, normalized)")) + 
    geom_line(aes(y = depression_norm, color = "Depression SVI (re-scaled, normalized)")) + 
...
hps_all_v6 %>% 
  ...
  summarize(across(c(hps_anxiety, anxiety_norm, stress_norm:worry_norm),
                   ~ weighted.mean(., pop))) %>% 
  group_by(start_date) %>% 
  mutate(google_average = mean(c(anxiety_norm:worry_norm))) %>% 
  ungroup() %>% 
  ggplot(aes(x = start_date)) + 
    geom_line(aes(y = hps_anxiety/1.2, color = "Household Pulse")) + 
    geom_line(aes(y = google_average/2, color = "Avg. related SVIs (re-scaled, normalized)")) + 
    geom_line(aes(y = anxiety_norm, color = "Anxiety SVI (re-scaled, normalized)")) + 
...
hps_all_v7 <- hps_all_v6 %>% 
  rowwise() %>% 
  mutate(depression_avg_norm = mean(c(depression_norm, crying_norm:sad_norm)),
         anxiety_avg_norm = mean(c(anxiety_norm, nervous_norm:worry_norm))) %>% 
  ungroup()
