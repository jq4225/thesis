...
hps_all_v8 %>% 
  ...
  ggplot(aes(x = start_date)) + 
    geom_line(aes(y = hps_depression, color = "Household Pulse")) + 
    geom_line(aes(y = google_average/1.5, color = "Avg. related SVIs (re-scaled, normalized)")) + 
    geom_line(aes(y = depression_norm/1.1, color = "Depression SVI (re-scaled, normalized)")) + 
...
hps_all_v8 %>% 
  ...
  ungroup() %>% 
  ggplot(aes(x = start_date)) + 
    geom_line(aes(y = lag(hps_anxiety, n = 6), color = "Household Pulse")) + 
    geom_line(aes(y = google_average/1.4, color = "Avg. related SVIs (re-scaled, normalized)")) + 
    geom_line(aes(y = anxiety_norm/1.6, color = "Anxiety SVI (re-scaled, normalized)")) + 
